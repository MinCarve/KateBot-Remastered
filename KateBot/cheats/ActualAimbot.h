...
public:
	...
	float GetSmoothValue(Vector& angles)
	{
		...
		int ID = LocalEntity.GetWeaponType(NULL);
		//
		switch (ID)
		{
		case EWeaponType::WeapType_Rifle:
			...
			cfg->AdaptiveSmoothMultiplier = cfg->RiflesAdaptiveSmoothMultiplier;
			break;
		case EWeaponType::WeapType_Pistol:
			if (cfg->PistolsAdaptiveSmooth)
				adaptive_smooth_enable = true;
			cfg->AdaptiveSmoothMultiplier = cfg->PistolsAdaptiveSmoothMultiplier;
			break;
		case EWeaponType::WeapType_SMG:
			if (cfg->SMGsAdaptiveSmooth)
				adaptive_smooth_enable = true;
			cfg->AdaptiveSmoothMultiplier = cfg->SMGsAdaptiveSmoothMultiplier;
			break;
		case EWeaponType::WeapType_Sniper:
			if (cfg->SnipersAdaptiveSmooth)
				adaptive_smooth_enable = true;
			cfg->AdaptiveSmoothMultiplier = cfg->SnipersAdaptiveSmoothMultiplier;
			break;
		case EWeaponType::WeapType_Shotgun:
			if (cfg->ShotGunsAdaptiveSmooth)
				adaptive_smooth_enable = true;
			cfg->AdaptiveSmoothMultiplier = cfg->ShotGunsAdaptiveSmoothMultiplier;
			break;
		case EWeaponType::WeapType_LMG:
			if (cfg->LMGsAdaptiveSmooth)
				adaptive_smooth_enable = true;
			cfg->AdaptiveSmoothMultiplier = cfg->LMGsAdaptiveSmoothMultiplier;
			break;
		default:
			break;
		}
